...
def load_data(file_buffer):
    # Use the fast, low-memory engine locally too!
    try:
...
def get_story_num(story_str):
    ...
    # Force Base to the absolute bottom and Roof to the absolute top
    if s == 'BASE': return -999 
    if 'ROOF' in s: return 999
    
    # 1. Check for Basement levels (e.g., "B1", "B2")
    m_b = re.search(r'B(\d+)', s)
    if m_b:
        return -int(m_b.group(1)) # Converts B1 to -1, B2 to -2, etc.
        
    # 2. Check for standard above-ground stories (e.g., "F1", "Story 2", "3")
    m = re.search(r'\d+', s)
    if m: 
        return int(m.group())
        
    return 0 # Default fallback for unrecognized text
...
for cfg in force_configs:
    ...
    # 1. Determine which limits to use for the color scale
    if use_custom:
        c_min_val = x_min
        c_max_val = x_max
    else:
